...
let AnnouncementsController = class AnnouncementsController {
    ...
    filesService;
    constructor(announcementsService, filesService) {
        this.announcementsService = announcementsService;
        this.filesService = filesService;
    }
    async create(createAnnouncementDto, req, file) {
        if (file) {
            const storedFile = await this.filesService.store(file, req.user.id, client_1.FileCategory.ANNOUNCEMENT);
    ...
    async update(id, updateAnnouncementDto, req, file) {
        if (file) {
            const storedFile = await this.filesService.store(file, req.user.id, client_1.FileCategory.ANNOUNCEMENT);
...
__decorate([
    ...
    __metadata("design:paramtypes", [create_announcement_dto_1.CreateAnnouncementDto, Object, Object]),
